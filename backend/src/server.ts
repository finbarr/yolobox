...
async function ensureMachineSSHCertificateTrust(
  ...
  await callAgentRPC(agent, "run_setup", {
    ...
    commands: [
      ...
      "sshd -t",
      "(systemctl reload ssh >/dev/null 2>&1 || systemctl reload sshd >/dev/null 2>&1 || systemctl restart ssh >/dev/null 2>&1 || systemctl restart sshd >/dev/null 2>&1 || pkill -HUP sshd >/dev/null 2>&1)",
    ],
  }, agentRPCDefaultTimeout);
}
