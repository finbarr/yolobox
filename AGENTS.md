...
- Remote tunnel SSH must use persistent host-key pinning, not `UserKnownHostsFile=/dev/null` or blanket host-key suppression. Otherwise every connect appears to accept a new host and real host identity drift is hidden.
- Remote CLI should stay thin. Workspace prep, setup commands, command wrapping, and tmux lifecycle belong behind backend/VM-agent RPC endpoints, not in CLI-generated shell scripts or broad machine PATCH calls.
